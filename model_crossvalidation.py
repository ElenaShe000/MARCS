...
# variables: n is length of floating window (15); m is the dimention of the output statistics (2); alpha is the level of statistical significance (0.5/0.10); t is array of t-values 2 sided Student test: https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.ttest_ind.html;  p is array of p-values to test difference in two mean values; years is selected years with p<alpha
 
def define_split_year(rr, n, m, alpha):
...
# 
alpha = 0.05
years = define_split_year(rr, 15, 2, alpha)
if not years:
   alpha = 0.10
   years = define_split_year(rr, 15, 2, alpha)
...
m1_tr = np.nanmean(rr.loc[rr.index[0]:split_year].values)
m2_tr = moment(rr.loc[rr.index[0]:split_year].values, moment=2, nan_policy='omit') + m1_tr * m1_tr
m3_tr = moment(rr.loc[rr.index[0]:split_year].values, moment=3,nan_policy='omit') + 3 * m2_tr * m1_tr - 2 * m1_tr * m1_tr * m1_tr
...
data = rr.loc[split_year+1:rr.index[numb-1]].values
normed_data = (data - np.nanmean(data))/np.nanstd(data) # sample 1: normed data series in the control period
r_con = pearson3.rvs(cs_con,size=len(rr.loc[split_year+1:rr.index[numb-1]].values)) #sample 2: generated by pearson3 with simulated cs for the control period
ks_con, pks_con = stats.ks_2samp(normed_data, r_con)

# and vice versa
m1_tr = np.nanmean(rr.loc[split_year+1:rr.index[numb-1]].values)
m2_tr = moment(rr.loc[split_year+1:rr.index[numb-1]].values, moment=2, nan_policy='omit') + m1_tr * m1_tr
m3_tr = moment(rr.loc[split_year+1:rr.index[numb-1]].values, moment=3,nan_policy='omit') + 3 * m2_tr * m1_tr - 2 * m1_tr * m1_tr * m1_tr
cv_tr = estimate_cv(m1_tr, m2_tr)
cs_tr = estimate_cs(m1_tr, m2_tr, m3_tr, cv_tr)

a_tr, b0_tr, b1_tr = basic_param_pearson(m1_tr, m2_tr, m3_tr) # basic parameterization
c_tr, gn_tr, gcn_tr =  basic_param_fpk(a_tr, b0_tr, b1_tr, pre_con)
a_con, b0_con, b1_con = simulate_param_pearson3(gcn_tr, gn_tr, c_tr, pre_tr) # constant parameters
# to simulate CV and CS for the Pearson type 3 distribution for the control period
cv_con, cs_con = simulate_skw(a_con, b0_con, b1_con)

data = rr.loc[rr.index[0]:split_year].values
normed_data = (data - np.nanmean(data))/np.nanstd(data)
r_tr = pearson3.rvs(cs_tr,size=len(rr.loc[rr.index[0]:split_year].values))
ks_tr, pks_tr = stats.ks_2samp(normed_data, r_tr)


# TO OUTPUT DATA TO THE RESULT FILE ############################################################
with open(WD + 'ModelCrossValidRes.txt', 'a') as f:
    f.write("%s %s %s %s %s %s %s\n" % (grdc, split_year, alpha, ks_tr, pks_tr, ks_con, pks_con))
